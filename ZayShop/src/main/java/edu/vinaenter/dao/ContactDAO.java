...
public class ContactDAO extends AbstractDAO<Contact> {
	...
	public List<Contact> getAll(){
		final String SQL = "SELECT * FROM contact ORDER BY ct_id DESC";
		return jdbcTemplate.query(SQL, new BeanPropertyRowMapper<>(Contact.class));
	}
	
	@Override
	public List<Contact> getAll(int offset, int rowCount){
		final String SQL = "SELECT * FROM contact ORDER BY ct_id DESC LIMIT ?,?";
		return jdbcTemplate.query(SQL, new BeanPropertyRowMapper<>(Contact.class) ,offset, rowCount);
	}
	
	@Override
	public int del(int id) {
		final String SQL = "DELETE FROM contact WHERE ct_id = ?";
		return jdbcTemplate.update(SQL, id);
	}
	
	@Override
	public int save(Contact t) {
		final String SQL = "INSERT INTO contact (ct_fullname, ct_email, ct_subject, ct_content) VALUES (?,?,?,?)";
	...
	public List<Contact> findAllByNameOderByNewName(String search) {
		final String SQL = "SELECT * FROM contact WHERE ct_fullname LIKE ?";
